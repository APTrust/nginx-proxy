...
or if you want to test the alpine flavor:
    ...
    docker build -t aptrust/nginx-proxy:test -f Dockerfile.alpine ..

make sure to tag that test image exactly `aptrust/nginx-proxy:test` or the test suite won't work.
...
Any domain name containing the substring `nginx-proxy` will resolve to the IP address of the container that was created from the `aptrust/nginx-proxy:test` image. So all the following domain names will resolve to the nginx-proxy container in tests:
- `nginx-proxy`
- `nginx-proxy.com`
- `www.nginx-proxy.com`

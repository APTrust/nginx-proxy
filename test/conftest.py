...
class requests_for_docker(object):
    ...
    def get_conf(self):
        ...
        """
        nginx_proxy_containers = docker_client.containers.list(filters={"ancestor": "aptrust/nginx-proxy:test"})
        if len(nginx_proxy_containers) > 1:
            pytest.fail("Too many running aptrust/nginx-proxy:test containers", pytrace=False)
        elif len(nginx_proxy_containers) == 0:
            pytest.fail("No running aptrust/nginx-proxy:test container", pytrace=False)
...
def nginx_proxy_dns_resolver(domain_name):
    ...
    issued from the docker image aptrust/nginx-proxy:test.
    ...
    if 'nginx-proxy' in domain_name:
        nginxproxy_containers = docker_client.containers.list(filters={"status": "running", "ancestor": "aptrust/nginx-proxy:test"})
        if len(nginxproxy_containers) == 0:
            log.warn("no container found from image aptrust/nginx-proxy:test while resolving %r", domain_name)
            return
...
def monkey_patch_urllib_dns_resolver():
    ...
    of the container created from image 'aptrust/nginx-proxy:test'.
    """
...
def wait_for_nginxproxy_to_be_ready():
    """
    If one (and only one) container started from image aptrust/nginx-proxy:test is found,
    wait for its log to contain substring "Watching docker events"
    """
    containers = docker_client.containers.list(filters={"ancestor": "aptrust/nginx-proxy:test"})
    if len(containers) != 1:
        return
...
def pytest_runtest_logreport(report):
    if report.failed:
        if isinstance(report.longrepr, ReprExceptionInfo):
            test_containers = docker_client.containers.list(all=True, filters={"ancestor": "aptrust/nginx-proxy:test"})
...
try:
    docker_client.images.get('aptrust/nginx-proxy:test')
except docker.errors.ImageNotFound:
    pytest.exit("The docker image 'aptrust/nginx-proxy:test' is missing")
